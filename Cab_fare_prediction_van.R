...
setwd("D:/EdwisorVanusha/Project/Cab Fare Prediction/Final")
getwd()
...
#we will limit the fare_amount to 1000 USD

train[which(train$fare_amount > 1000 ),]
nrow(train[which(train$fare_amount > 1000),])
...
#Findings:
#1.As we can see that in train dataset we find missing values in passenger_count-55 (0.35%) and fare_amount- 25 (0.14%). 
...
#Plot of number of missing values in the trin dataset
